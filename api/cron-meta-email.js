...
function generatePdf(rows, date, timeIST, slot) {
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
  const SYM = c => c === 'THB' ? 'THB ' : c === 'NZD' ? 'NZD ' : 'INR ';
  ...
  doc.text('merakiads — Meta Intelligence Report', 14, 14);

  // Slot + meta line
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(80, 80, 80);
  const slotLabel = slot === 'morning' ? 'Morning Report' : 'Evening Report';
  const active = rows.filter(r => parseFloat(r.spend) > 0).length;
  const totalINR = rows.filter(r => r.currency === 'INR').reduce((s, r) => s + parseFloat(r.spend || 0), 0);
  doc.text(`${slotLabel}   |   Date: ${date}   |   Time: ${timeIST} IST   |   Active Clients: ${active}/${rows.length}   |   Total INR Spend: INR ${Math.round(totalINR).toLocaleString('en-IN')}`, 14, 30);
  ...
  autoTable(doc, {
    startY: 44,
    head: [['Client', 'Currency', 'Spend', 'Impressions', 'CTR%', 'Results']],
    body: rows.map(r => [
      r.name,
      r.currency,
      parseFloat(r.spend || 0) > 0 ? `${SYM(r.currency)}${Math.round(parseFloat(r.spend)).toLocaleString('en-IN')}` : '—',
      parseInt(r.impressions || 0) > 0 ? parseInt(r.impressions).toLocaleString('en-IN') : '—',
      parseFloat(r.ctr || 0) > 0 ? parseFloat(r.ctr).toFixed(2) + '%' : '—',
      r.results || '—',
    ]),
    theme: 'grid',
    headStyles: { fillColor: [125, 194, 66], textColor: [255, 255, 255], fontStyle: 'bold', fontSize: 8 },
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(30, 30, 30);
  doc.text('Alerts & Issues', 14, yPos);
  yPos += 6;
  ...
  if (alertRows.length === 0) {
    ...
    doc.text('✓ No alerts — all accounts healthy', 18, yPos + 5.5);
  } else {
    for (const r of alertRows) {
      if (yPos > 270) { doc.addPage(); yPos = 20; }

      // Client name bar
      doc.setFillColor(240, 247, 230);
      doc.rect(14, yPos, 182, 7, 'F');
      doc.setDrawColor(125, 194, 66);
      doc.rect(14, yPos, 2, 7, 'F');
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(30, 30, 30);
      doc.text(r.name, 18, yPos + 5);
      yPos += 9;

      const items = [];
      if (['DISABLED','GRACE PERIOD'].includes(r.accountStatus))
        items.push({ color: [220, 38, 38], msg: `Account ${r.accountStatus}` });
      else if (r.accountStatus && r.accountStatus !== 'OK')
        items.push({ color: [217, 119, 6], msg: `Account ${r.accountStatus}` });
      if (r.rejectedAds)
        items.push({ color: [220, 38, 38], msg: `Rejected Ads: ${r.rejectedAds.slice(0, 100)}` });
      if (r.zeroSpendAlert)
        items.push({ color: [217, 119, 6], msg: r.zeroSpendAlert });
      if (r.budgetAlert)
        items.push({ color: [217, 119, 6], msg: r.budgetAlert });
  ...
  doc.text(`Generated by Meraki Ads Meta Intelligence  •  ${date} ${timeIST} IST  •  ${SHEET_URL}`, 14, 287);
...
function buildHtml(rows, date, timeIST, slot) {
  ...
  for (const r of rows) {
    ...
    else if (r.accountStatus && r.accountStatus !== 'OK') warningCount++;
  ...
  const tableRows = rows.map(r => {
    ...
    const spendFmt = spend > 0 ? `<strong>${sym}${Math.round(spend).toLocaleString('en-IN')}</strong>` : `<span style="color:#bbb">—</span>`;
    const impFmt = parseInt(r.impressions || 0) > 0 ? parseInt(r.impressions).toLocaleString('en-IN') : `<span style="color:#bbb">—</span>`;
    const ctrFmt = parseFloat(r.ctr || 0) > 0 ? parseFloat(r.ctr).toFixed(2) + '%' : `<span style="color:#bbb">—</span>`;
    const hasCritical = r.rejectedAds || ['DISABLED','GRACE PERIOD'].includes(r.accountStatus);
    const hasWarning = r.zeroSpendAlert || r.budgetAlert || ['UNSETTLED','PENDING'].includes(r.accountStatus);
  ...
  const alertRows = rows.filter(r => r.rejectedAds || r.zeroSpendAlert || r.budgetAlert || (r.accountStatus && r.accountStatus !== 'OK'));
  const alertsHtml = alertRows.length === 0
    ...
    : alertRows.map(r => {
        const items = [];
        if (['DISABLED','GRACE PERIOD'].includes(r.accountStatus)) items.push({ sev: 'critical', icon: '⛔', msg: `Account ${r.accountStatus}` });
        else if (r.accountStatus && r.accountStatus !== 'OK') items.push({ sev: 'warning', icon: '⚠️', msg: `Account ${r.accountStatus}` });
        if (r.rejectedAds) items.push({ sev: 'critical', icon: '🚫', msg: `Rejected: ${r.rejectedAds.slice(0, 120)}` });
        if (r.zeroSpendAlert) items.push({ sev: 'warning', icon: '📉', msg: r.zeroSpendAlert });
        if (r.budgetAlert) items.push({ sev: 'warning', icon: '💰', msg: r.budgetAlert });
  ...
  ${tokenWarningHtml}
  <div style="text-align:center;padding:8px;font-size:10px;color:#bbb;">Meraki Ads Internal · ${date} · PDF report attached</div>
</div>`;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let pdfBase64 = null;
    try {
      pdfBase64 = generatePdf(rows, today, timeIST, slot);
    } catch (pdfErr) {
      console.error('PDF generation failed:', pdfErr.message);
    }

    const criticalCount = rows.filter(r => r.rejectedAds || ['DISABLED','GRACE PERIOD'].includes(r.accountStatus)).length;
    ...
    await transporter.sendMail({
      ...
      attachments: pdfBase64 ? [{
        filename: `Meraki-Meta-${slot.charAt(0).toUpperCase() + slot.slice(1)}-${today}-${timeIST.replace(':', '')}.pdf`,
        content: Buffer.from(pdfBase64, 'base64'),
        contentType: 'application/pdf',
      }] : [],
    });

    return res.status(200).json({ success: true, date: today, timeIST, slot, rowsRead: rows.length, pdfAttached: !!pdfBase64 });
  } catch (err) {
