...
//   featuredPosts?  — posts to render in "Social Media Content" (defaults
//                     to ALL of `posts`, not a top-N cutoff — a month
//                     with 30 posts gets a 30-post grid across as many
//                     pages as needed)
...
export async function generateSocialReportPdf(data) {
  ...
  const gridPosts = data.featuredPosts && data.featuredPosts.length ? data.featuredPosts : allPosts
  ...
  await drawContentGridPages(doc, 'Social Media Content', gridPosts, palette)
