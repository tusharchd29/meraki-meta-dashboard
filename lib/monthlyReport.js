...
export async function buildMonthlyReportData(targetMonth) {
  ...
  // A roster entry with no budget, no live Meta connection, no imported
  // Google spend, and no error to explain why is just noise — it was never
  // wired up for this month at all. Drop it instead of printing a blank
  // page; anything with real signal (budget, spend, campaigns, or an error
  // worth investigating) still shows.
  const activeReports = clientReports.filter(c =>
    c.budget != null || c.spentInr > 0 ||
    c.metaCampaigns.length > 0 || c.googleCampaigns.length > 0 ||
    c.metaError
  );

  return { clientReports: activeReports, label, targetMonth };
}
