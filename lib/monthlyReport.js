...
// A campaign row with no spend and no result is inactive-in-this-period
// noise (typically a legacy campaign from an older CSV import that still
// exists in the sheet). Keeping every such row is what turned Volvo's
// Google table into 60+ lines of "0 0 Conversions". Drop them, but return
// the count so the report can say how many were hidden rather than pretend
// they don't exist.
function activeCampaignRows(list) {
  const kept = [];
  let hidden = 0;
  for (const r of list) {
    if (r.spend === 0 && !(r.resultCount > 0)) hidden++;
    else kept.push(r);
  }
  return { kept, hidden };
}
...
function hasNoActivity(c) {
  return c.budget == null && c.spentInr === 0 && c.metaCampaigns.length === 0 &&
...
export function buildBrandedPdf(clientReports, monthLabel, droppedDuplicates = []) {
  ...
  const sortedForSummary = [...activeReports].sort((a, b) =>
    urgencyRank(a) - urgencyRank(b) || b.spentInr - a.spentInr
  );
  ...
  doc.text(`${monthLabel}   |   ${activeReports.length} client${activeReports.length === 1 ? '' : 's'} with activity or a budget set`, 14, 32);

  autoTable(doc, {
    startY: 38,
    head: [['Client', 'Budget', 'Spent (INR)', 'Pacing']],
    body: sortedForSummary.map(c => [
      c.name,
      c.budget != null ? `Rs ${Math.round(c.budget).toLocaleString('en-IN')}` : 'not set',
      `Rs ${Math.round(c.spentInr).toLocaleString('en-IN')}`,
      c.pace ? `${c.pace.replace('_', ' ')} (${c.finalPct != null ? c.finalPct.toFixed(0) : '—'}%)` : (c.spentInr > 0 ? 'no budget set' : '—'),
    ]),
    theme: 'grid',
  ...
  activeReports.forEach((c) => {
    ...
    if (metaRows.length) {
      ...
      doc.setFont('helvetica', 'bold');
      const metaIsForeign = c.metaCurrency && c.metaCurrency !== 'INR';
      ...
      doc.setTextColor(140, 140, 140);
      if (metaIsForeign) {
        const metaTotal = metaRows.reduce((s, m) => s + m.spend, 0);
        const rate = fxRates[c.metaCurrency?.toUpperCase()];
        const inrText = rate != null
          ? `≈ Rs ${Math.round(metaTotal * rate).toLocaleString('en-IN')} at 1 ${c.metaCurrency} = Rs ${rate}`
          : `no INR conversion rate configured for ${c.metaCurrency}`;
        doc.text(inrText, 14, y);
        y += 4;
      }
      if (metaHidden) {
        doc.text(`+ ${metaHidden} campaign${metaHidden === 1 ? '' : 's'} with no spend this period not shown.`, 14, y);
        y += 4;
      }
      y += 4;
    } else if (c.metaError) {
    ...
    if (googleRows.length) {
      ...
      doc.setFont('helvetica', 'bold');
      const googleIsForeign = c.googleCurrency && c.googleCurrency !== 'INR';
      ...
      doc.setTextColor(140, 140, 140);
      if (googleIsForeign) {
        const googleTotal = googleRows.reduce((s, g) => s + g.spend, 0);
        const rate = fxRates[c.googleCurrency?.toUpperCase()];
        const inrText = rate != null
          ? `≈ Rs ${Math.round(googleTotal * rate).toLocaleString('en-IN')} at 1 ${c.googleCurrency} = Rs ${rate}`
          : `no INR conversion rate configured for ${c.googleCurrency}`;
        doc.text(inrText, 14, gy);
        gy += 4;
      }
      if (googleHidden) {
        doc.text(`+ ${googleHidden} campaign${googleHidden === 1 ? '' : 's'} with no spend this period not shown.`, 14, gy);
      }
    } else {
      doc.setFontSize(8);
